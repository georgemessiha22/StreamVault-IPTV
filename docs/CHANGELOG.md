...
- Fixed the in-app updater for beta builds: the beta build sequence is now baked into the installed version (e.g. `1.1.18-beta.000002.01`) and the updater compares beta sequences, so a beta install reliably detects and downloads the latest beta pre-release and no longer offers an "update" to the beta it is already running.
